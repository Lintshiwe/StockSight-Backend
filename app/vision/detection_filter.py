...
DetectionMode = Literal["all_objects", "strict_warehouse_retail", "retail_coco_fallback", "safety"]
...
class DetectionFilterSettings:
    detection_mode: DetectionMode = "all_objects"
...
class DetectionFilter:
    ...
    def apply(self, detections: list[Detection]) -> list[Detection]:
        ...
        return [
            ...
            if detection.class_name not in blocked and (not allowed or detection.class_name in allowed)
        ]

    def allowed_class_set(self) -> set[str]:
        if self.settings.detection_mode == "all_objects":
    ...
    def status(self, loaded_class_names: dict[int, str] | dict[str, str] | None = None) -> dict[str, object]:
        ...
        return {
            ...
            "allowed_classes": sorted(allowed) if allowed else sorted(loaded),
            "blocked_classes": sorted(blocked),
            "matching_loaded_classes": matching if allowed else sorted(loaded),
            "blocked_loaded_classes": blocked_loaded,
            "filter_warning": None
            if (matching or not allowed)
